...
### Course Start Date : Aug 4th, 2014
...
CheckDir("dataCousera")         ## create direcory called 'dataCousera' to download data from url given
setwd("./dataCousera")          ## set working directory to 'dataCousera' directory
...
download.file(zipfileurl, destfile) ## download the file to destination file
...
traindata[,562] = read.table("UCI HAR Dataset/train/Y_train.txt", sep="", header=FALSE)
traindata[,563] = read.table("UCI HAR Dataset/train/subject_train.txt", sep="", header=FALSE)

                                 ## Read the data from testing set file
testdata = read.table("./UCI HAR Dataset/test/X_test.txt", sep="", header=FALSE)
testdata[,562] = read.table("UCI HAR Dataset/test/Y_test.txt", sep="", header=FALSE)
testdata[,563] = read.table("UCI HAR Dataset/test/subject_test.txt", sep="", header=FALSE)

one_dataset = rbind(traindata, testdata)  ## combined training and testing data into merged one data set
